...
const getFullImageUrl = (imageUrl: string): string => {
  ...
  const LOCAL_IP = '192.168.31.18'; // Замените на свой IP
...
export default function HomeScreen() {
  ...
  const userId = 1;
  const baseUrl =
    Platform.OS === 'android' ? 'http://10.0.2.2:8080' : 'http://192.168.31.18:8080';
  ...
  const handlePaymentPress = () => {
    ...
    router.push({ pathname: '/finance/paymentForServices', params: { billId: currentBill.id } });
  };
        ...
        <ThemedCard style={{ marginTop: 65, padding: 16 }}>
          <ThemedView withBackground={false} style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
            <ThemedView withBackground={false} style={{ flexShrink: 1 }}>
              ...
              {currentBill.period && (
                <ThemedText type="label" style={{ marginTop: 2, fontSize: 12, color: '#8A8A8A' }}>
                  Период: {formatPeriod(currentBill.period)}
                </ThemedText>
              )}
            </ThemedView>
